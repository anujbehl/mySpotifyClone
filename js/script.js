...
async function getSongs(folder){
    ...
    for (let index = 0; index < folderSongs.length; index++){
        if (folderSongs[index].innerHTML.includes("mp3"))
            songs.push(folderSongs[index].innerHTML.split("\"")[1].split(".")[0])
    }
...
async function displayAlbums() {
    ...
    let div = document.createElement("div")
    div.innerHTML = response;
    let songsList = div.getElementsByTagName("script")
    let cardContainer = document.querySelector(".cardContainer")
    let finalSongs= []
    for (let index = 0; index < songsList.length; index++){
        if (songsList[index].innerHTML.startsWith("addRow") && !songsList[index].innerHTML.includes("htaccess"))
            finalSongs.push(songsList[index])
    }
